...
class MLP(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.mlp2_lif(x)
        x = self.mlp2_conv(x.flatten(0, 1)) #计算缓慢
        x = self.mlp2_bn(x).reshape(T, B, C, H, W)
        return x

#这里是计算量最密集的地方，如果要降低计算量，要么使用加法做注意力计算，要么降低输入进来的长宽值
class SpikingSelfAttention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., sr_ratio=1):
        ...
        self.proj_bn = nn.BatchNorm1d(dim)# 为什么一个BN计算速度那么慢

    def forward(self, x):
        T, B, C, H, W = x.shape
        x = self.proj_lif(x)

        x = x.flatten(3)# 将H*W融合为一个维度
        T, B, C, N = x.shape
        x_for_qkv = x.flatten(0, 1) # 融合T*B为一个维度
        ...
        attn = (q @ k.transpose(-2, -1))# 计算速度及其缓慢
        x = (attn @ v) * 0.125

        x = x.transpose(3, 4).reshape(T, B, C, N)
        x = self.attn_lif(x) #计算及其缓慢debug模式下要运算几分钟
        x = x.flatten(0, 1) #
        x = self.proj_bn(self.proj_conv(x)).reshape(T, B, C, H, W) #同上
        return x


class SpikingTransformer(nn.Module):# SpikingTransformer块
...
class SpikingTokenizer(nn.Module):# 此处的SpikingTokenizer还是有点问题，降维的太少了，看看后续如何改进
...
#     input = torch.randn(2, 3, 160, 160).cuda()
#     model = create_model('spikingformer',
#                                   pretrained=False,
#                                   drop_rate=0,
#                                   drop_path_rate=0.1,
